...
parser.add_argument('--model_dir', type=str, default='./test_using_models/generator_forward_75_cycleoffi_hue0.2G1.pth', help='trained model')
...
def dice_coeff(seg, real_seg):
    ...
    P =TP + FN
    N =FP + TN
    # return (np.sum(seg[real_seg == 1])*2.0 + smooth) / (np.sum(seg) + np.sum(real_seg) + smooth) #origin
    SN = TP/(TP+FN)
    # return SN
...
# 测试单纯使用UNET分割
    ...
    test_dataloader = DataLoader(DRIVEDataset(os.path.join(opt.root, "dataset/%s" % opt.dataset_name), transformsA=transforms_stronga, transformsB=transforms_strongb, mode='train'),
