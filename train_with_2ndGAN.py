...
# name of training
parser.add_argument('--net_name', type=str, default='official Unet 2ndGAN',
                    help='the name of training this time, part of the subdirectory of save and image')

# choose generator and discriminator
generator = UNet(drop=0.3, in_channels=1)
discriminator = PixelDiscriminator(input_nc=2)
...
img_dealing_method = "rotate crop flip nonorm"
seg_dealing_method = "rotate crop flip nonorm"
...
def choose_transforms_method(mode = "default"):
    ...
    elif mode == "rotate crop flip nonorm":
        trans = [transforms.Resize((opt.img_height, opt.img_width), Image.BICUBIC),
    ...
    elif mode == "lightness rotate crop flip nonorm":
        trans = [transforms.Resize((opt.img_height, opt.img_width), Image.BICUBIC),
...
dataloader = DataLoader(DRIVEDataset(os.path.join(opt.root, "dataset/%s" % opt.dataset_name), mode = "train", transformsA=transforms_methodA, transformsB=transforms_methodB,is2ndtrain =True),
                        batch_size=opt.batch_size, shuffle=False, num_workers=opt.n_cpu)

# print('len of train batch is: ', len(dataloader))
#validation used for test cnd calculate loss
val_dataloader = DataLoader(
    DRIVEDataset(os.path.join(opt.root, "dataset/%s" % opt.dataset_name), mode='val',transformsA=transforms_val, transformsB=transforms_val,is2ndtest =True),
    batch_size=1, shuffle=False, num_workers=opt.n_cpu)

val_dataloader_for_mid = DataLoader(DRIVEDataset(os.path.join(opt.root, "dataset/%s" % opt.dataset_name), mode='val',transformsA=transforms_methodA, transformsB=transforms_methodB,is2ndtest =True),
    batch_size=opt.batch_size, shuffle=False, num_workers=opt.n_cpu)
...
for epoch in range(opt.start_epoch, opt.end_epoch+1):
    ...
    print(" [current epoch average: D loss: %f, G loss: %f] [Accuracy: %f] [already use time: %s time left: %s] [lr %f]" %
          (Dlosses_current_epoch_average, Glosses_current_epoch_average, acc, time_used,time_left,optimizer_G.state_dict()['param_groups'][0]['lr']))
