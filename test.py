...
parser.add_argument('--syn_seg_threshed_dir',  type=str, default='./test_results/syn_threshed_seg/', help='save generated segmented images under directory')
parser.add_argument('--model_dir', type=str, default='./test_using_models/generator_forward_100.pth', help='trained model')
...
def make_threshed(norm_img_path, thresh, threshed_img_dir):
    ...
    labels = np.where(labels > thresh, 1, 0)
...
# 测试单纯使用UNET分割
    ...
    # transforms_stronga = [transforms.Resize((opt.img_height, opt.img_width), Image.BICUBIC),
    #          # transforms.Grayscale(num_output_channels=1),
    #          transforms.RandomRotation(180),
    ...
    #          transforms.RandomResizedCrop(512, (0.008, 1)),
    #          transforms.RandomVerticalFlip(),
    #          transforms.ToTensor(),
    #          transforms.Normalize(0.5, 0.5)
    #          ]
    # transforms_strongb = [transforms.Resize((opt.img_height, opt.img_width), Image.BICUBIC),
    #          # transforms.Grayscale(num_output_channels=1),
    #          transforms.RandomRotation(180),
    #          transforms.ColorJitter(brightness=0.7, contrast=0.7, saturation=0.7, hue=0.2),
    #          # transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.05),
    ...
    #          transforms.Normalize(0.5, 0.5)
    #          ]
    test_dataloader = DataLoader(DRIVEDataset(os.path.join(opt.root, "dataset/%s" % opt.dataset_name), transformsA=transforms_as, transformsB=transforms_bs, mode='test'),
...
def testt_accuracy():
    ...
    for real_seg_path in glob.glob(os.path.join(opt.real_seg_dir, '*.jpg'), recursive=True):
        thresh =30   # grayscale intensity
        dice = evaluationMetric(real_seg_path, thresh, opt.syn_seg_dir)
        print(real_seg_path +" dice: "+str(dice))
