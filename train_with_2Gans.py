...
# name of training
parser.add_argument('--net_name', type=str, default='2GANs hardtanh lambda0.99',
                    help='the name of training this time, part of the subdirectory of save and image')

# choose generator and discriminator
generator_forward = GeneratorUnetW(drop=0.3, in_channels=1)
generator_backward = GeneratorUnetW(drop=0.3, in_channels=1)
...
if opt.start_epoch != 0:
    ...
    Glosses_forward = list(np.load(os.path.join(save_dir + ' Glosses_forward_all.npy')))
    Glosses_backward = list(np.load(os.path.join(save_dir + 'Glosses_backward_all.npy')))
    ...
    Glosses_forward_epoch_average = list(np.load(os.path.join(save_dir + 'Glosses_forward_epoch_average.npy')))
    Glosses_backward_epoch_average = list(np.load(os.path.join(save_dir + 'Glosses_backward_epoch_average.npy')))
...
# np.save(os.path.join(save_dir + 'training_time_by_epoch.npy'), elapse)
    ...
    Glosses_forward = []#by every training
    Glosses_backward = []#by every training
    Dlosses_forward = []
    Dlosses_backward = []
    Glosses_forward_epoch_average = []#average by every epoch
    Glosses_backward_epoch_average = []#average by every epoch
...
# Optimizers
optimizer_G1 = torch.optim.Adam(generator_forward.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
optimizer_G2 = torch.optim.Adam(generator_backward.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
...
for epoch in range(opt.start_epoch, opt.end_epoch+1):
    Glosses_forward_epoch_temp =[]
    Glosses_backward_epoch_temp =[]
    ...
    for i, pair in enumerate(dataloader):
        ...
        # Total loss
        loss_G1 = (1 - lambda_pixel) * loss_GAN_forward + lambda_pixel * loss_pixel_forward
        ...
        fake_C = generator_backward(fake_B)
        pred_fake_backward = discriminator_backward(real_B, fake_C)
        loss_GAN_backward = criterion_GAN(pred_fake_backward, valid)  # 均方根误差损失函数
        loss_pixel_backward = criterion_pixelwise(fake_C, real_B)  # 平均绝对误差损失函数
        loss_G2 = (1 - lambda_pixel) * loss_GAN_backward + lambda_pixel * loss_pixel_backward
        loss_G2.backward()
        optimizer_G2.step()
        # ---------------------
        #  训练判别器1
        ...
        # Fake loss
        pred_fake_backward = discriminator_backward(real_B, fake_C.detach())  # 虚假的
        ...
        # Print log
        sys.stdout.write("\r[Epoch %d/%d Batch %d/%d] [inGloss: D1: %f, P1: %f, D2: %f, P2: %f] [D1 loss: %f, D2 loss: %f, G1 loss: %f, G2 loss: %f]" %  #, pixel: %f, adv: %f
                         (epoch, opt.end_epoch,
                          i, len(dataloader),
                          loss_GAN_forward , loss_pixel_forward, loss_GAN_backward , loss_pixel_backward,
                          loss_D_forward.item(), loss_D_backward.item(), loss_G1.item(), loss_G2.item()
                          # loss_pixel_forward.item(), loss_GAN_forward.item(),
                          ))
        # record every train (20 record for 20 images in a epoch)
        Glosses_forward.append(loss_G1.item())
        Glosses_backward.append(loss_G2.item())
        Dlosses_forward.append(loss_D_forward.item())
        Dlosses_backward.append(loss_D_backward.item())
        Glosses_forward_epoch_temp.append(loss_G1.item())
        Glosses_backward_epoch_temp.append(loss_G2.item())
    ...
    #record every epoch
    Glosses_forward_current_epoch_average = sum(Glosses_forward_epoch_temp) / float(len(Glosses_forward_epoch_temp))
    Glosses_backward_current_epoch_average = sum(Glosses_backward_epoch_temp) / float(len(Glosses_backward_epoch_temp))
    ...
    Glosses_forward_epoch_average.append(Glosses_forward_current_epoch_average)
    Glosses_backward_epoch_average.append(Glosses_backward_current_epoch_average)
    ...
    print(" [current epoch average: D1 loss: %f, D2 loss: %f, G1 loss: %f, G2 loss: %f] [Accuracy: %f] [already use time: %s time left: %s] [lr %f]" %
          (Dlosses_forward_current_epoch_average, Dlosses_backward_current_epoch_average, Glosses_forward_current_epoch_average,Glosses_backward_current_epoch_average, acc, time_used, time_left, optimizer_G1.state_dict()['param_groups'][0]['lr']))

    if epoch != 0 and epoch % opt.lrdecay == 0:
        optimizer_G1 = torch.optim.Adam(generator_forward.parameters(), lr=opt.lr / (10 ** lrdecay_times), betas=(opt.b1, opt.b2))
    ...
    if opt.checkpoint_interval != -1 and epoch != 0 and epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(generator_forward.state_dict(), os.path.join(save_dir + 'generator_backward_%d.pth' % epoch))
        torch.save(discriminator_forward.state_dict(), os.path.join(save_dir + 'discriminator_backward_%d.pth' % epoch))
        np.save(os.path.join(save_dir + 'Glosses_all.npy'), Glosses_forward)
        np.save(os.path.join(save_dir + 'Dlosses_forward_all.npy'), Dlosses_forward)
        np.save(os.path.join(save_dir + 'Dlosses_backward_all.npy'), Dlosses_backward)
        np.save(os.path.join(save_dir + 'Glosses_epoch_average.npy'), Glosses_forward_epoch_average)
        np.save(os.path.join(save_dir + 'Dlosses_forward_epoch_average.npy'), Dlosses_forward_epoch_average)
        np.save(os.path.join(save_dir + 'Dlosses_backward_epoch_average.npy'), Dlosses_backward_epoch_average)
        np.save(os.path.join(save_dir + 'accuracies.npy'), accuracies)
        np.save(os.path.join(save_dir + 'training_time_by_epoch.npy' ), elapse)
